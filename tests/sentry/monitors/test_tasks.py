...
class MonitorDetectBrokenMonitorEnvTaskTest(TestCase):
    ...
    @mock.patch("sentry.monitors.tasks.MessageBuilder")
        ...
        expected_contexts = [
            {
                "broken_monitors": [
                    (
                        monitor.slug,
                        f"https://{self.organization.slug}.sentry.io/crons/{self.project.slug}/{monitor.slug}/?environment={self.environment.name}",
                        timezone.now() - timedelta(days=14),
                    )
                ],
                "view_monitors_link": f"https://{self.organization.slug}.sentry.io/crons",
            },
            {
                "broken_monitors": [
                    (
                        second_monitor.slug,
                        f"https://{second_org.slug}.sentry.io/crons/{second_project.slug}/{second_monitor.slug}/?environment={second_env.name}",
                        timezone.now() - timedelta(days=14),
                    )
                ],
                "view_monitors_link": f"https://{second_org.slug}.sentry.io/crons",
            },
            {
                "broken_monitors": [
                    (
